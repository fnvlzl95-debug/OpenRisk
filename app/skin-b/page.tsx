...
export default function SkinBHome() {
  ...
  const selectSuggestion = (suggestion: SearchSuggestion) => {
    ...
    setSelectedIndex(-1)
    setIsLoading(true)
    router.push(`/skin-b/result?query=${encodeURIComponent(suggestion.name)}`)
  }
      ...
      <main className="relative z-10 pt-20 pb-10">
        <div className="relative flex flex-col items-center justify-center min-h-[80vh] px-4 w-full max-w-4xl mx-auto transition-all duration-700">
          ...
          <div className={`text-center transition-all duration-500 ${isFocused ? 'opacity-40 scale-95 blur-sm' : 'opacity-100'}`}>
            <h1 className="text-5xl sm:text-7xl font-bold tracking-tight mb-4 bg-clip-text text-transparent bg-gradient-to-b from-white to-white/60">
              상권의 진실을<br />마주하세요.
            </h1>
          ...
          >
            <form onSubmit={handleSearch} className="relative group">
              ...
              <div className="relative flex items-center bg-[#111] border border-white/10 rounded-2xl p-2 shadow-2xl">
                ...
                <input
                  ...
                  placeholder="지역명, 지하철역, 번지수 입력..."
                  className="w-full bg-transparent h-14 text-xl outline-none placeholder:text-white/20 text-white font-medium"
                  autoComplete="off"
                />
                <button
                  type="submit"
                  disabled={isLoading || !query.trim()}
                  className="px-6 py-3 bg-white text-black font-bold rounded-xl hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {isLoading ? (
                    <>
              ...
              {showSuggestions && (
                <div className="absolute top-full left-0 right-0 mt-4 bg-[#111]/90 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl z-50">
                  <div className="px-4 py-3 text-xs font-mono text-white/30 border-b border-white/5 flex justify-between">
                  ...
                  {suggestions.map((item, index) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-sm text-white/40">{item.district}</div>
                      </div>
                      <svg className={`w-4 h-4 text-white/40 transform transition-transform ${index === selectedIndex ? 'translate-x-1 text-white' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {/* Quick Tags */}
            {!showSuggestions && (
              <div className={`mt-6 flex flex-wrap justify-center gap-2 transition-opacity duration-500 ${isFocused ? 'opacity-0' : 'opacity-100'}`}>
                {['홍대입구역', '성수동', '강남역', '이태원'].map((tag) => (
                  <button
                    key={tag}
                    onClick={() => { setQuery(tag); }}
                    className="px-4 py-1.5 rounded-full text-sm font-medium bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/20 text-white/60 hover:text-white transition-all"
                  >
                    {tag}
                  </button>
                ))}
          ...
          {/* Footer Stats / Legend */}
          <div className={`absolute bottom-10 left-0 right-0 px-6 transition-all duration-700 ${isFocused ? 'translate-y-20 opacity-0' : 'translate-y-0 opacity-100'}`}>
            <div className="max-w-4xl mx-auto border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-white/40">
              <div className="flex gap-6">
                {GRADE_DATA.map((g) => (
                  <div key={g.grade} className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${g.color}`} />
                    <span className="font-mono text-white/70">{g.label}</span>
                  </div>
                ))}
              </div>
              <div className="font-mono text-xs">
                <span className="text-white">51</span> AREAS ANALYZED
              </div>
            </div>
          </div>
