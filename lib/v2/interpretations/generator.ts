...
export function generateSummary(
  ...
  if (compLevel === 'high' && trafficLevel === 'low' && costLevel === 'high') {
    return '경쟁은 치열한데 손님은 없고 임대료는 비쌉니다. 여기서 버틸 수 있는 구조가 아닙니다.'
  }

  // 경쟁 낮음 + 유동 낮음 (함정)
  if (compLevel === 'low' && trafficLevel === 'low') {
    return '경쟁자가 없는 이유가 있습니다. 수요 자체가 없을 가능성이 높습니다.'
  }
  ...
  // 위험 신호 3개 이상
  if (badCount >= 3) {
    return '경쟁, 비용, 생존율 모두 불리합니다. 시작부터 수비하는 싸움입니다.'
  }

  // 경쟁 높음 + 유동 높음
  if (trafficLevel === 'high' && compLevel === 'high') {
    return '사람은 많지만 나눠 가질 가게도 많습니다. 차별점 없으면 체력전에서 밀립니다.'
  }

  // 임대료 높음 + 폐업률 높음
  if (costLevel === 'high' && survivalLevel === 'high') {
    return '못 버티고 나가는 가게가 많은 구간입니다. 임대료를 감당 못 한 흔적입니다.'
  }

  // 임대료 높음
  if (costLevel === 'high') {
    return '매달 나가는 고정비가 큽니다. 예상 매출로 버틸 수 있는지부터 확인하세요.'
  }

  // 유동 낮음
  if (trafficLevel === 'low') {
    return '지나가는 손님이 거의 없습니다. 스스로 손님을 끌어와야 하는 자리입니다.'
  }

  // 경쟁 높음
  if (compLevel === 'high') {
    return '이미 자리 잡은 가게들과 싸워야 합니다. 신규는 불리한 게임입니다.'
  }

  // 폐업률 높음
  if (survivalLevel === 'high') {
    return '주변 가게들이 자주 바뀝니다. 오래 버틴 곳이 드문 구간입니다.'
  }

  // 기본
  return '특별히 나쁘진 않지만 특별히 좋지도 않습니다. 결국 실행력 싸움입니다.'
}
