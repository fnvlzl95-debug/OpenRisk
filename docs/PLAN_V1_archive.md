...
### 1.3 핵심 차별점
기존 상권분석 서비스가 "데이터(그래프/표)"를 보여주는 데 그친다면,
오픈리스크는 **"초보자가 이해할 수 있는 리스크 번역"**을 제공한다.

> "사람이 많다 = 좋다" ❌
> "사람은 많은데, 초보가 버티기 어려운 구조인지" ⭕
...
### PostGIS가 필수인 이유
```
사용자: "마포구 서교동 357-1" 검색

❌ PostGIS 없음:
   → "서교동(행정동)" 전체 통계
   → 홍대 메인거리 + 조용한 주택가 섞임 = 데이터 쓰레기

⭕ PostGIS 있음:
   → 이 주소가 "서교동 카페거리 상권(폴리곤)" 안에 있는지 0.01초 만에 판별
   → 정확한 상권 단위 분석
```
...
│   └── geo/
│       └── matcher.ts          # 좌표 → 상권 매칭 (PostGIS)
├── scripts/
│   └── uploadData.ts           # CSV → DB 업로드 스크립트
└── .env.local                  # API 키들
```
...
## 7. UX Writing (등급별 핵심 문장)

### A_주거 (Safe Zone)
- "유행보다는 '매일 필요한 것'을 찾는 곳입니다."
- "큰 대박보다는 월급 같은 꾸준한 매출이 발생합니다."
- "초보 창업자에게 가장 리스크가 낮은 '방어형 상권'입니다."

### B_혼합 (Gray Zone)
- "주거민과 직장인이 섞여 있어 타겟팅이 모호할 수 있습니다."
- "점심과 저녁, 평일과 주말의 분위기가 완전히 다릅니다."
- "유동인구가 많아 보이지만, 내 가게로 들어온다는 보장은 없습니다."
...
### D_특수 (Special)
- "일반적인 상권 이론이 통하지 않는 특수 지역입니다."
- "특정 시설(병원, 학교, 산업단지)의 상황에 따라 매출이 결정됩니다."
- "가게의 노력보다 외부 환경 요인이 절대적입니다."
...
## 10. Lv4 신호 설계 (Phase 2)

### 10.1 밥/술 상권 (daypart_profile)
| 유형 | 특징 |
|------|------|
| MEAL | 주간 피크 + 짧은 체류 + 낮은 객단가 |
| ALCOHOL | 야간 유동 + 긴 체류 + 높은 객단가 |
| MIXED | 시간대 분리 해석 필요 |

### 10.2 광고 신호
| 신호 | 설명 | 경험 근거 |
|------|------|----------|
| ALI (광고 레버리지) | 광고가 성장 레버가 될 가능성 | C등급: 매출 1000→5000 (2개월) |
| ADI (광고 의존도) | 광고 없으면 흔들리는 구조 | 술집은 광고로 좌우됨 |
| MCP (마케팅 비용 압박) | 노출비용이 과도 | 제주도 플레이스 원가 1000~2000 |

**핵심 원칙**:
> "C는 광고로 성장할 수 있지만 비용전쟁이고, A는 광고보다 단골 구조가 승부다."
...
2. New Project 생성
3. Database → Extensions → `postgis` 활성화
...
- [ ] lib/geo/matcher.ts (PostGIS ST_Contains)
...
### 서울 상권 데이터
- **상권영역(폴리곤)**: 서울시 상권분석서비스 - 영역-상권
- **추정매출**: 서울시 상권분석서비스 - 추정매출-상권
- **점포/개폐업**: 서울시 상권분석서비스 - 점포-상권
