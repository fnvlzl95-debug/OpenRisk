...
| 4 | **생존 신호** | 개업/폐업 트렌드 기반 | grid_store_counts (closure/opening) | closure-risk.ts |
...
실제 개업/폐업 데이터 기반으로 상권 건강도를 측정합니다.
...
| 트렌드 | 조건 | 설명 |
|--------|------|------|
| `growing` | openingRate > closureRate | 점포가 늘어나는 시장 |
| `shrinking` | closureRate > openingRate | 점포가 줄어드는 시장 |
| `stable` | openingRate ≈ closureRate | 안정적인 시장 |

#### 리스크 레벨 기준
| 레벨 | 폐업률 | 설명 |
|------|--------|------|
| `low` | ≤ 30% | 생존율 높은 지역 |
| `medium` | 31~50% | 일반적인 수준 |
| `high` | > 50% | 생존율 낮은 지역 |

#### 데이터 산출 방식
- **비교 기간**: 2024.12 → 2025.10 (10개월)
- **개업**: 2025.10에만 존재하는 점포
- **폐업**: 2024.12에만 존재하는 점포
- **단위**: H3 그리드 (resolution 9, ~174m)
