...
### 3차 데이터 (소상공인시장진흥공단)
- **상가업소정보 CSV** (2024.12, 2025.10)
  - H3 그리드(resolution 9)로 변환
  - 개업/폐업 비교 분석에 사용
...
### 5. grid_store_counts (H3 그리드별 점포 데이터)

v2 분석에서 사용하는 핵심 테이블

| 컬럼 | 타입 | 설명 |
|------|------|------|
| h3_index | varchar | H3 셀 ID (resolution 9) |
| category | varchar | 업종 카테고리 |
| total_count | int | 해당 카테고리 점포 수 |
| period | varchar | 데이터 기준 기간 |
| closure_count | int | 폐업 점포 수 (v2.1) |
| opening_count | int | 개업 점포 수 (v2.1) |

**폐업/개업 데이터 산출:**
- 비교 기간: 2024.12 → 2025.10
- 폐업: 2024.12에만 존재하던 점포
- 개업: 2025.10에 새로 등장한 점포
...
### v2.1 완료 (2026-01-03)
- ✅ 생존 신호 트렌드 (growing/stable/shrinking)
- ✅ 폐업/개업 데이터 임포트 API
- ✅ 19개 업종별 특화 해석 문장
