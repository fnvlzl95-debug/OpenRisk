...
-- profiles: 모두 조회 가능, 본인만 수정 (is_admin 변경 불가!)
...
-- posts: 삭제되지 않은 글만 조회, 로그인 유저 작성, 본인만 수정/삭제
-- is_notice는 관리자만 true 가능
CREATE POLICY "posts_select" ON public.posts FOR SELECT USING (deleted_at IS NULL);
...
-- view_count는 서버에서만 증가 (RPC 함수 사용)
CREATE OR REPLACE FUNCTION public.increment_view_count(post_id INTEGER)
RETURNS void AS $$
BEGIN
...
-- comments: 삭제되지 않은 댓글만 조회
CREATE POLICY "comments_select" ON public.comments FOR SELECT USING (deleted_at IS NULL);
...
-- 5. 신규 사용자 프로필 자동 생성 트리거 (에러 핸들링 포함)
...
DECLARE
  ...
  v_avatar TEXT;
BEGIN
  -- 카카오 메타데이터 안전하게 추출 (키가 없어도 에러 안남)
  v_kakao_id := (NEW.raw_user_meta_data->>'provider_id')::BIGINT;
  v_nickname := COALESCE(
...
| # | 취약점 | 해결 방법 | 상태 |
|---|--------|----------|------|
| 1 | `profiles_update` 정책에서 `is_admin` 임의 변경 가능 | WITH CHECK 절에서 is_admin 변경 방지 조건 추가 | 반영됨 |
| 2 | `posts_select`에서 숨김 게시글 노출 가능성 | 현재 is_hidden 미사용, 필요시 추가 | 보류 |
| 3 | 일반 사용자가 `is_notice=true`로 공지글 작성 가능 | INSERT/UPDATE 정책에 관리자 체크 추가 | 반영됨 |
| 4 | `handle_new_user()` 트리거 메타데이터 키 의존성 | 여러 키 fallback + EXCEPTION 핸들링 추가 | 반영됨 |

### 8.0.1 Should-fix 항목

| # | 항목 | 설명 | 결정 |
|---|------|------|------|
| 5 | view_count 조작 가능 | RLS로 증가 로직 제한 또는 API에서만 처리 | API에서 처리 |
| 6 | comments에 updated_at 없음 | 댓글 수정 시 시간 추적 불가 | 컬럼 추가됨 |
| 7 | Rate Limiting 구체화 필요 | 미들웨어 vs Edge Function | 미들웨어로 결정 |
